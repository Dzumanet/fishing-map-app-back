...
import {CreateFishResponse, GetListOfFishResponse, RegisterUserResponse} from "../types";
...
export class FishService {
    ...
    async addFish(createFish: CreateFishDto, user: User): Promise<CreateFishResponse> {

        const {fishName, weight,description,catchDateTime, lat, lon} = createFish;
    ...
    async updateFish(id: string, updateFish: CreateFishDto): Promise<Fish> {
        const {fishName, weight,description,catchDateTime, lat, lon} = updateFish;
    ...
    async getUserFish(userId: string): Promise<Fish[]> {
        const user: RegisterUserResponse = await this.userService.getOneUser(userId);
        return Fish.find({
            where: {user}
        });
    }
